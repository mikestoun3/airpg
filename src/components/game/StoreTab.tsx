    ...
    <div className="max-w-2xl mx-auto space-y-4">
      ...
      {result && (
        <div className="fixed inset-0 z-50 flex items-center justify-center bg-black/70 px-4" onClick={() => setResult(null)}>
          <div className="bg-[#111118] border border-[rgba(200,70,70,0.3)] rounded-2xl p-6 max-w-sm w-full text-center space-y-4" onClick={e => e.stopPropagation()}>
            <div className="text-4xl">🎉</div>
            <p className="text-slate-200 font-bold text-lg">Purchase Successful!</p>
            <p className="text-[#78788a] text-sm">{result.name}</p>
            ...
            {result.reward.type === 'loot_case' && result.item && (
              <div className={`border ${RARITY_BORDER[result.item.rarity]} ${RARITY_BG[result.item.rarity]} ${RARITY_GLOW[result.item.rarity]} rounded-xl px-4 py-3`}>
                ...
                <p className="text-[#505058] text-xs mt-1 capitalize">{result.item.rarity} · GS {result.item.gearScore}</p>
      ...
      {/* Header */}
      <div className="bg-[#111118] border border-[rgba(255,255,255,0.09)] rounded-xl p-4">
        <div className="flex items-center gap-3">
          <span className="text-2xl">🏬</span>
          <div className="flex-1">
            <p className="text-slate-200 font-bold">Store</p>
            <p className="text-[#606068] text-xs">Purchase Gold, Essence, and Loot Cases with POL (Polygon)</p>
          </div>
      ...
      <div className="grid grid-cols-1 sm:grid-cols-2 gap-3">
        {filtered.map(item => {
          ...
          return (
            <div key={item.id} className="bg-[#111118] border border-[rgba(200,70,70,0.18)] rounded-xl p-4 flex flex-col gap-3 relative">
              {item.badge && (
              ...
              <div className="flex items-center gap-3">
                <div className="text-3xl w-12 h-12 flex items-center justify-center bg-[#080818] rounded-xl border border-[rgba(255,255,255,0.06)]">
                  {item.icon}
                </div>
                <div className="flex-1 min-w-0">
                  <p className="text-slate-200 font-semibold text-sm">{item.name}</p>
                  <p className="text-[#606068] text-xs">{item.description}</p>
                </div>
              </div>
                    ...
                    .map(([rarity, weight]) => {
                      ...
                      return (
                        <div key={rarity} className="text-center">
                          ...
                          <p className="text-[9px] text-[#44444e] capitalize truncate">{rarity.slice(0, 4)}</p>
                        </div>
                      );
                    })}
                </div>
      ...
      <p className="text-center text-[#404048] text-xs pb-2">
