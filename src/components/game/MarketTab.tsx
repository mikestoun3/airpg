...
export function MarketTab({ state, onRefresh }: Props) {
  ...
  return (
    <div className="flex flex-col md:flex-row gap-3 md:gap-6 md:h-full md:overflow-hidden">
      ...
      {/* Mobile tab switcher */}
      <div className="flex md:hidden gap-1 flex-shrink-0 bg-[#130909] border border-[rgba(200,70,70,0.15)] rounded-xl p-1">
        <button onClick={() => setMobileTab('browse')}
          className={`flex-1 py-2 rounded-lg text-xs font-semibold transition-all ${
            mobileTab === 'browse' ? 'bg-[#1e0e0e] text-slate-100' : 'text-[#5a3535]'
          }`}>
          🏪 Browse
        </button>
        <button onClick={() => setMobileTab('sell')}
          className={`flex-1 py-2 rounded-lg text-xs font-semibold transition-all ${
            mobileTab === 'sell' ? 'bg-[#1e0e0e] text-slate-100' : 'text-[#5a3535]'
          }`}>
          💰 Sell
        </button>
      </div>
        ...
        {state.walletAddress ? (
          <>
            <div className="bg-[#180c0c] border border-[rgba(200,70,70,0.2)] rounded-xl p-3">
              <p className="text-[10px] text-[#6a4040] mb-1">Your wallet</p>
              <p className="text-slate-300 text-xs font-mono">{shortAddr(state.walletAddress)}</p>
              <p className="text-[10px] text-[#4a3030] mt-1">Marketplace fee: <span className="text-amber-400/70">{MARKET_FEE_PCT}%</span> · Your payout: <span className="text-emerald-400/70">{100 - MARKET_FEE_PCT}%</span></p>
            </div>

            <p className="text-[11px] text-[#5a3535] uppercase tracking-widest">Select item to list</p>
            <div className="space-y-1.5">
              {state.inventory.length === 0 && (
                <p className="text-xs text-[#4a3030] italic px-1">No items in inventory</p>
              )}
              {state.inventory.map((item) => (
                <button
                  key={item.id}
                  onClick={() => { setSelectedItem(item); setShowListModal(true); }}
                  className={`w-full flex items-center gap-2 p-2.5 rounded-lg bg-[#130909] border ${RARITY_BORDER[item.rarity]} hover:bg-[#180c0c] transition-all text-left`}>
                  <div className="flex-1 min-w-0">
                    <p className="text-xs font-semibold truncate" style={{ color: RARITY_COLORS[item.rarity] }}>
                      {item.name}
                    </p>
                    <p className="text-[10px] text-[#6a4040]">{item.slot} · GS {item.gearScore}</p>
                  </div>
                  <span className="text-[#6a4040] text-lg shrink-0">→</span>
                </button>
              ))}
            </div>
            ...
            {myListings.length > 0 && (
              <>
                <p className="text-[11px] text-[#5a3535] uppercase tracking-widest mt-2">Your listings</p>
                <div className="space-y-1.5">
                  {myListings.map((l) => {
                    ...
                    return (
                    <div key={l.id}
                      className={`p-2.5 rounded-lg bg-[#130909] border ${RARITY_BORDER[l.item.rarity]}`}>
                      <div className="flex items-center gap-2">
                        <div className="flex-1 min-w-0">
                          ...
                          {locked && (
                            <p className="text-[9px] text-[#6a4040]">🔒 cancel in {lockedMins}m</p>
                          )}
                        </div>
                        <button
                          onClick={() => { setEditingId(isEditing ? null : l.id); setEditPrice(''); }}
                          className="text-[10px] text-[#6a4040] hover:text-red-400 transition-colors shrink-0 px-1"
                          title="Edit price">
                          ✎
                        </button>
                        <button
                          ...
                          className="text-[10px] text-[#6a4040] hover:text-red-400 transition-colors shrink-0 disabled:opacity-30 disabled:cursor-not-allowed">
                          {cancellingId === l.id ? '…' : '✕'}
                        </button>
                      </div>
                      {isEditing && (
                        <div className="mt-2 flex gap-1.5">
                          <input
                            ...
                            className="flex-1 min-w-0 bg-[#180c0c] border border-[rgba(200,70,70,0.25)] rounded-lg px-2 py-1 text-slate-200 text-xs outline-none focus:border-red-600"
                          />
                          <button
                          ...
                          <button
                            ...
                            className="px-2 py-1 text-[10px] rounded-lg text-[#5a3535] hover:text-slate-300 transition-colors">
                            ✕
                          </button>
                        </div>
                      )}
          ...
          <div className="bg-[#180c0c] border border-[rgba(200,70,70,0.2)] rounded-xl p-4 text-center">
            <p className="text-[#6a4040] text-sm">Connect MetaMask to sell items</p>
          </div>
        )}
      </div>
      ...
      <div className={`${mobileTab === 'browse' ? 'flex' : 'hidden'} md:flex flex-1 flex-col gap-3 overflow-hidden`}>
        ...
        <div className="flex items-center justify-between">
          ...
          <div className="flex items-center gap-3">
            ...
            <button onClick={fetchListings} className="text-[11px] text-[#6a4040] hover:text-[#b07070] transition-colors">
              ↺ Refresh
            </button>
          </div>
        </div>
        ...
        {/* Filter bar */}
        <div className="flex-shrink-0 bg-[#130909] border border-[rgba(200,70,70,0.12)] rounded-xl overflow-x-auto">
        <div className="flex gap-3 items-center px-4 py-3 min-w-max">
          {/* Rarity toggles */}
          <div className="flex items-center gap-1.5 flex-wrap">
            <span className="text-[10px] text-[#4a3030] uppercase tracking-wider mr-0.5">Rarity</span>
            <button
              onClick={() => setFilterRarities(new Set())}
              className={`px-2.5 py-1 rounded-lg text-[11px] font-medium transition-all border ${
                filterRarities.size === 0
                  ? 'bg-[#1e0e0e] text-slate-300 border-[rgba(200,70,70,0.3)]'
                  : 'text-[#4a3030] border-transparent hover:text-[#7a5050]'
              }`}>
              All
            </button>
            {RARITY_ORDER.map(r => {
              ...
              return (
                <button key={r} onClick={() => toggleRarity(r)}
                  className={`px-2.5 py-1 rounded-lg text-[11px] font-medium capitalize transition-all border ${
                    active
                      ? 'bg-[#1e1010] border-current'
                      : 'border-transparent hover:bg-[#180c0c]'
                  }`}
          ...
          {/* Divider */}
          <div className="w-px h-5 bg-[rgba(200,70,70,0.15)] self-center" />

          {/* Slot filter */}
          <div className="flex items-center gap-2">
            <span className="text-[10px] text-[#4a3030] uppercase tracking-wider">Slot</span>
            <select value={filterSlot} onChange={e => setFilterSlot(e.target.value as EquipmentSlot | '')}
              className="bg-[#180c0c] border border-[rgba(200,70,70,0.2)] rounded-lg px-2 py-1 text-slate-300 text-xs outline-none focus:border-purple-600">
          ...
          {/* Price range */}
          <div className="flex items-center gap-2">
            <span className="text-[10px] text-[#4a3030] uppercase tracking-wider">Price</span>
            <input value={minPol} onChange={e => setMinPol(e.target.value)}
              type="number" min="0" placeholder="min"
              className="w-20 bg-[#180c0c] border border-[rgba(200,70,70,0.2)] rounded-lg px-2 py-1 text-slate-300 text-xs outline-none focus:border-purple-600 placeholder:text-[#3a3a60]"
            />
            ...
            <input value={maxPol} onChange={e => setMaxPol(e.target.value)}
              ...
              className="w-20 bg-[#180c0c] border border-[rgba(200,70,70,0.2)] rounded-lg px-2 py-1 text-slate-300 text-xs outline-none focus:border-purple-600 placeholder:text-[#3a3a60]"
            />
            <span className="text-[10px] text-[#4a3030]">POL</span>
          </div>
          ...
          {/* Sort */}
          <div className="flex items-center gap-2">
            <span className="text-[10px] text-[#4a3030] uppercase tracking-wider">Sort</span>
        ...
        {loading ? (
          <div className="flex-1 flex items-center justify-center text-[#4a3030]">Loading…</div>
        ) : otherListings.length === 0 ? (
          <div className="flex-1 flex flex-col items-center justify-center text-center">
            <p className="text-4xl mb-3">🏪</p>
            <p className="text-[#6a4040]">{hasFilters ? 'No listings match your filters' : 'No listings right now'}</p>
            {hasFilters
              ? <button onClick={() => { setFilterRarities(new Set()); setFilterSlot(''); setMinPol(''); setMaxPol(''); }}
          ...
          <div className="grid grid-cols-1 sm:grid-cols-2 gap-3 content-start">
            {otherListings.map((l) => {
              ...
              return (
                <div key={l.id}
                  className={`bg-[#180c0c] rounded-xl border ${RARITY_BORDER[l.item.rarity]} overflow-hidden flex flex-col`}>
                  ...
                  <div className="p-4 flex flex-col gap-2.5 flex-1">
                    <div>
                      ...
                      <p className="text-[#6a4040] text-[10px]">{l.item.slot} · GS {l.item.gearScore}</p>
                    </div>

                    <div className="bg-[#130909] rounded-lg p-2 text-xs">
                      <div className="flex justify-between text-[#5a3535]">
                      ...
                      {l.item.secondaryStats?.map((s) => (
                        <div key={s.stat} className="flex justify-between text-[#6a4040]">
                    ...
                    <div className="flex items-center justify-between">
                      <div>
                        ...
                        <p className="text-[10px] text-[#4a3030]">{shortAddr(l.sellerWallet)} · {MARKET_FEE_PCT}% fee</p>
                      </div>
                      <button
                        onClick={() => handleBuy(l)}
          ...
          {!hasFilters && listings.length < total && (
            <button onClick={loadMore} disabled={loadingMore}
              className="w-full py-2.5 rounded-xl bg-[#180c0c] border border-[rgba(200,70,70,0.2)] text-[#7a5050] text-sm hover:text-slate-300 hover:border-[rgba(200,70,70,0.35)] disabled:opacity-50 transition-all">
      ...
      {showListModal && selectedItem && (
        <div className="fixed inset-0 bg-black/70 backdrop-blur-sm flex items-center justify-center z-50 p-6">
          <div className="bg-[#130909] border border-[rgba(200,70,70,0.25)] rounded-2xl w-full max-w-sm overflow-hidden shadow-2xl">
            <div className="px-6 py-5 border-b border-[rgba(200,70,70,0.12)]">
              <h2 className="text-slate-100 font-bold text-lg">List on Market</h2>
              <p className="text-[#6a4040] text-xs mt-0.5">Price in POL — buyer pays you directly on Polygon</p>
            </div>
            <div className="px-6 py-5 space-y-4">
              <div className={`bg-[#180c0c] rounded-xl border ${RARITY_BORDER[selectedItem.rarity]} p-3`}>
                ...
                <p className="text-[#6a4040] text-xs">{selectedItem.slot} · GS {selectedItem.gearScore} · +{selectedItem.primaryValue} {selectedItem.primaryStat}</p>
              </div>

              <div>
                <label className="text-[11px] text-[#5a3535] uppercase tracking-widest block mb-2">
                  Price (POL)
                </label>
                <input
                  type="number"
                  ...
                  className="w-full bg-[#180c0c] border border-[rgba(200,70,70,0.2)] rounded-xl px-4 py-2.5 text-slate-100 text-sm focus:outline-none focus:border-red-600/50"
                />
                {pricePol && !isNaN(parseFloat(pricePol)) && parseFloat(pricePol) > 0 ? (
                  <div className="mt-2 bg-[#130909] rounded-lg px-3 py-2 text-xs space-y-1">
                    <div className="flex justify-between text-[#6a4040]">
                    ...
                    <div className="flex justify-between font-semibold border-t border-[rgba(200,70,70,0.1)] pt-1">
                      <span className="text-[#7a5050]">You receive</span>
                      <span className="text-emerald-400">{(parseFloat(pricePol) * (100 - MARKET_FEE_PCT) / 100).toFixed(4)} POL</span>
                    </div>
                  </div>
                ) : (
                  <p className="text-[10px] text-[#4a3030] mt-1">{MARKET_FEE_PCT}% marketplace fee applies</p>
                )}
              </div>
            </div>
            <div className="px-6 py-4 border-t border-[rgba(200,70,70,0.10)] flex gap-3">
              <button
                onClick={() => { setShowListModal(false); setSelectedItem(null); setPricePol(''); }}
                className="flex-1 py-2.5 rounded-xl border border-[rgba(200,70,70,0.15)] text-[#7a5050] text-sm hover:text-slate-300 transition-colors">
                Cancel
              </button>
              <button
                onClick={handleList}
