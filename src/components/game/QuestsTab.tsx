...
export function QuestsTab() {
  ...
  return (
    ...
    <div className="max-w-2xl mx-auto space-y-4 pb-4">
      ...
      {/* Section tabs */}
      <div className="flex gap-2 p-1 bg-[#0e0e14] rounded-xl border border-[rgba(255,255,255,0.07)]">
        {([
          ['social', '🌐 Social'],
          ['referral', '👥 Referrals'],
          ['dungeons', '🗺 Dungeons'],
      ...
      {activeSection === 'social' && (
        <div className="space-y-3">
          <p className="text-[#606068] text-xs px-1">Complete tasks and claim your rewards. These are verified on honor system.</p>
          {socialQuests.map(quest => {
            ...
            return (
              <div key={quest.id} className={`bg-[#111118] border rounded-xl p-4 flex items-center gap-4 transition-all ${done ? 'border-emerald-700/30 opacity-70' : 'border-[rgba(255,255,255,0.09)]'}`}>
                ...
                <div className="flex-1 min-w-0">
                  ...
                  <p className="text-[#606068] text-xs mt-0.5">{quest.description}</p>
      ...
      {activeSection === 'referral' && (
        <div className="space-y-3">
          {/* Invite link card */}
          <div className="bg-[#111118] border border-[rgba(255,255,255,0.09)] rounded-xl p-4 space-y-3">
            <div className="flex items-center gap-2">
              <span className="text-lg">🔗</span>
              <div>
                <p className="text-slate-200 font-semibold text-sm">Your Invite Link</p>
                <p className="text-[#606068] text-xs">Friends who join via your link get +300 Gold & +30 ESS. You get +200 Gold & +20 ESS per referral.</p>
              </div>
            </div>
            <div className="flex items-center gap-2 bg-[#080818] rounded-lg px-3 py-2 border border-[rgba(255,255,255,0.07)]">
              <span className="text-[#505058] text-xs font-mono flex-1 truncate">{inviteLink}</span>
              <button onClick={copyLink}
                className={`shrink-0 px-3 py-1 text-xs rounded-lg font-semibold transition-all ${copied ? 'bg-emerald-700 text-white' : 'bg-[#1c1c28] hover:bg-[#252550] text-[#848490]'}`}>
                {copied ? '✓ Copied!' : 'Copy'}
              </button>
            </div>
            <div className="flex items-center gap-2">
          ...
          {referralQuests.map(quest => {
            ...
            return (
              <div key={quest.id} className={`bg-[#111118] border rounded-xl p-4 transition-all ${done ? 'border-emerald-700/30 opacity-70' : canClaim ? 'border-amber-600/40' : 'border-[rgba(255,255,255,0.08)]'}`}>
                ...
                {!done && (
                  <div className="mb-3">
                    <div className="flex items-center justify-between text-[11px] mb-1">
                      <span className="text-[#606068]">Progress</span>
                ...
                <div className="flex justify-end">
                  {done ? (
                    ...
                    <span className="text-[#44444e] text-xs">Invite {needed - progress} more</span>
                  )}
                </div>
              </div>
            );
      ...
      {activeSection === 'dungeons' && (
        <div className="space-y-5">
          <p className="text-[#606068] text-xs px-1">Reach milestone floors in each dungeon to earn rewards. Progress is saved from your deepest run.</p>
              ...
              <div key={dungeonName}>
                ...
                <div className="space-y-2">
                  {quests.map(quest => {
                    ...
                    return (
                      <div key={quest.id}
                        className={`bg-[#111118] border rounded-xl p-3.5 transition-all ${done ? 'border-emerald-700/30 opacity-70' : canClaim ? 'border-amber-600/40' : 'border-[rgba(255,255,255,0.08)]'}`}>
                        <div className="flex items-center gap-3">
                          ...
                          <div className="flex-1 min-w-0">
                            ...
                            <p className="text-[#606068] text-xs mt-0.5">Floor {required}</p>
                          </div>
                          <div className="shrink-0">
                            {done ? (
                              ...
                              <span className="text-[#44444e] text-xs tabular-nums">{floor}/{required}</span>
                            )}
                          </div>
                        </div>
                        {!done && floor < required && (
