        ...
        <div className="bg-[#14142a] border border-[rgba(120,110,200,0.2)] rounded-xl p-4">
          ...
          <div className="grid grid-cols-3 md:grid-cols-1 gap-3 md:gap-0 md:space-y-3">
            {[
              ...
              { icon: '🔮', label: 'Essence', value: character.essence, color: 'text-purple-400' },
              { icon: '⭐', label: 'Relics', value: character.relics, color: 'text-blue-400' },
            ].map(({ icon, label, value, color }) => (
              <div key={label} className="flex flex-col md:flex-row md:items-center md:justify-between bg-[#0f0f22] md:bg-transparent rounded-lg md:rounded-none p-2 md:p-0 text-center md:text-left">
                <span className="text-[#8080b0] text-xs flex flex-col md:flex-row items-center gap-1 md:gap-2">
                  <span>{icon}</span> <span className="hidden md:inline">{label}</span>
                  <span className="md:hidden text-[10px] text-[#6060a0]">{label}</span>
                </span>
        ...
        <div className="bg-[#14142a] border border-[rgba(120,110,200,0.2)] rounded-xl p-4">
          ...
          <div className="flex items-center gap-2">
            <div className="flex-1 flex gap-1">
              {campUpgrades.map((u) => (
                <div key={u.id} className={`flex-1 h-1.5 rounded-full ${u.built ? 'bg-purple-500' : 'bg-[#252545]'}`} />
              ))}
            </div>
            ...
            return (
              <div key={upgrade.id}
                className={`bg-[#14142a] border rounded-xl overflow-hidden transition-all ${
                  ...
                  : prereqMet ? 'border-[rgba(120,110,200,0.2)] hover:border-[rgba(120,110,200,0.35)]'
                    ...
                    <div className="flex items-center justify-between">
                      <div className="flex gap-2 text-xs">
                        ...
                        {upgrade.essenceCost > 0 && (
                          <span className={character.essence >= upgrade.essenceCost ? 'text-purple-400' : 'text-red-400'}>{upgrade.essenceCost} ess</span>
                        )}
                      </div>
                      {!prereqMet ? (
                        <span className="text-xs text-[#4040a0]">Needs: {campUpgrades.find(u => u.id === upgrade.requires)?.name}</span>
                      ) : (
