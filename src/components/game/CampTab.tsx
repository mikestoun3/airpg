  ...
  return (
    <div className="flex flex-col md:flex-row gap-4 md:gap-6 md:h-full">
      ...
      <div className="w-full md:w-64 md:flex-shrink-0 flex flex-col gap-4">
        ...
        <div className="bg-[#180c0c] border border-[rgba(200,70,70,0.2)] rounded-xl p-4">
          ...
          <div className="grid grid-cols-3 md:grid-cols-1 gap-3 md:gap-0 md:space-y-3">
            {[
              ...
              <div key={label} className="flex flex-col md:flex-row md:items-center md:justify-between bg-[#130909] md:bg-transparent rounded-lg md:rounded-none p-2 md:p-0 text-center md:text-left">
                <span className="text-[#b07070] text-xs flex flex-col md:flex-row items-center gap-1 md:gap-2">
                  <span>{icon}</span> <span className="hidden md:inline">{label}</span>
                  <span className="md:hidden text-[10px] text-[#5a3535]">{label}</span>
                </span>
        ...
        {/* Progress */}
        <div className="bg-[#180c0c] border border-[rgba(200,70,70,0.2)] rounded-xl p-4">
          <p className="text-[11px] text-[#5a3535] uppercase tracking-widest mb-3">Progress</p>
          <div className="flex items-center gap-2">
            ...
            <span className="text-[#5a3535] text-xs">{builtIds.length}/{campUpgrades.length}</span>
          </div>
        </div>
        ...
        <div className="bg-[#180c0c] border border-[rgba(200,70,70,0.2)] rounded-xl p-4">
          ...
          <form onSubmit={handlePromoRedeem} className="flex gap-2">
            <input
              ...
              className="flex-1 bg-[#0c0c20] border border-[rgba(200,70,70,0.2)] rounded-lg px-3 py-2 text-slate-200 text-xs font-mono uppercase tracking-wider outline-none focus:border-purple-600 placeholder:text-[#3535a0] placeholder:normal-case placeholder:tracking-normal"
            />
      ...
      <div className="flex-1 md:overflow-y-auto">
        ...
        <div className="grid grid-cols-1 sm:grid-cols-2 gap-4">
          {campUpgrades.map((upgrade) => {
            ...
            return (
              <div key={upgrade.id}
                className={`bg-[#180c0c] border rounded-xl overflow-hidden transition-all ${
                  upgrade.built ? 'border-purple-600/30'
                  : prereqMet ? 'border-[rgba(200,70,70,0.2)] hover:border-[rgba(200,70,70,0.35)]'
                  : 'border-[rgba(200,70,70,0.08)] opacity-55'
                }`}>
                ...
                <div className="p-4 md:p-5">
                  <div className="flex items-start gap-3 mb-3">
                    ...
                    <div>
                      ...
                      <p className="text-[#5a3535] text-xs mt-0.5">{upgrade.description}</p>
                    </div>
                  </div>
                  <div className="bg-[#130909] rounded-lg px-3 py-2 mb-4">
                  ...
                  {!upgrade.built && (
                    <div className="flex items-center justify-between">
                      ...
                      {!prereqMet ? (
                        <span className="text-xs text-[#4a3030]">Needs: {campUpgrades.find(u => u.id === upgrade.requires)?.name}</span>
                      ) : (
