...
function Code({ children }: { children: React.ReactNode }) {
  return <code className="bg-[#111118] border border-[rgba(255,255,255,0.09)] rounded px-1.5 py-0.5 text-red-300 text-xs font-mono">{children}</code>;
}
...
function Row({ label, value, sub }: { label: string; value: string; sub?: string }) {
  return (
    <div className="flex items-start justify-between py-2 border-b border-[rgba(255,255,255,0.05)] last:border-0">
      <span className="text-[#78788a] text-sm">{label}</span>
      <div className="text-right">
        <span className="text-slate-200 text-sm font-mono">{value}</span>
        {sub && <div className="text-[#606068] text-[10px]">{sub}</div>}
      </div>
    </div>
  );
}
  ...
  return (
    <div className="flex flex-col md:flex-row gap-4 md:gap-6 md:h-full">
          ...
          <button key={s.id} onClick={() => setSection(s.id)}
            className={`flex-shrink-0 flex items-center gap-1.5 px-3 py-1.5 rounded-full text-xs border transition-all ${
              section === s.id
                ...
                : 'text-[#505058] border-[rgba(255,255,255,0.08)] bg-[#16161f]'
            }`}>
            <span>{s.icon}</span>
      ...
      <div className="hidden md:block w-48 flex-shrink-0">
        <div className="bg-[#16161f] border border-[rgba(255,255,255,0.09)] rounded-xl p-2 space-y-0.5 sticky top-0">
          {SECTIONS.map(s => (
            <button key={s.id} onClick={() => setSection(s.id)}
              className={`w-full flex items-center gap-2.5 px-3 py-2 rounded-lg text-sm transition-all text-left ${
                section === s.id
                  ? 'bg-[#1c1c28] text-slate-100 border border-[rgba(200,70,70,0.25)]'
                  : 'text-[#505058] hover:bg-[#1c1c28] hover:text-[#a0a0a8]'
              }`}>
      ...
      <div className="flex-1 overflow-y-auto pr-0 md:pr-1">
        <div className="bg-[#16161f] border border-[rgba(255,255,255,0.09)] rounded-xl p-4 md:p-6 max-w-3xl">
          ...
          {section === 'overview' && (
            <div>
              ...
              <div className="bg-[#111118] rounded-xl p-4 space-y-0">
          ...
          {section === 'stats' && (
            <div>
              ...
              <Box color="purple">
                ...
                <div className="bg-[#111118] rounded-lg p-3 mt-2">
              ...
              <Box color="blue">
                ...
                <p className="text-[#505058] text-xs text-center mt-1">Example: PWR 8, END 5 → CR = floor(12 + 5) = 17</p>
              </Box>
            </div>
          )}
            ...
            <div>
              ...
              <Box color="blue">
                ...
                <div className="mt-3 space-y-1">
                  <p className="text-[#505058] text-xs text-center">CR = DC → 50% odds · CR = DC+30 → 80% odds · CR = DC+45 → 95% (cap)</p>
                </div>
              </Box>

              <H3>Examples</H3>
              <div className="bg-[#111118] rounded-xl p-4 space-y-3 text-sm text-[#9090b8]">
                <div>
          ...
          {section === 'floors' && (
            <div>
              ...
              <Box color="purple">
                ...
                <p className="text-[#505058] text-xs text-center mt-1">Example: Goblin Warrens floor 5 = round(5 + 4 × 2.5) = 15</p>
              </Box>
              ...
              <Box color="amber">
                ...
                <p className="text-[#505058] text-xs text-center mt-1">Floor 10 = +72% gold · Floor 20 = +152% gold</p>
              </Box>
          ...
          {section === 'items' && (
            <div>
              ...
              <div className="space-y-2">
                {[
                  ...
                  <div key={r.tier} className="bg-[#111118] rounded-xl overflow-hidden">
                    <div className="px-3 py-2 border-b border-[rgba(255,255,255,0.05)] flex items-center gap-3">
                      <span className={`font-bold text-sm ${r.color}`}>T{r.tier} {r.name}</span>
                      <span className="text-[10px] text-[#606068]">GS {r.gs} · {r.primary} primary · {r.secondary} secondary</span>
                    </div>
                    <div className="px-3 py-2 text-xs text-[#78788a] flex flex-col sm:flex-row gap-2">
                      <span>🔥 <span className="text-[#848490]">{r.forge}</span></span>
                      <span>⚒ Absorbs: <span className="text-[#848490]">{r.req}</span></span>
                    </div>
                  </div>
                ))}
              </div>
              ...
              <Box color="amber">
                ...
                <div className="bg-[#111118] rounded-lg p-3 mt-2 space-y-0">
                ...
                <div key={d.name} className="mb-4 bg-[#111118] rounded-xl overflow-hidden border border-[rgba(255,255,255,0.07)]">
                  ...
                  <div className="p-4">
                    <div className="flex items-center gap-2 mb-3">
                      ...
                      <span className="text-[10px] text-[#606068] bg-[#16161f] px-1.5 py-0.5 rounded border border-[rgba(255,255,255,0.08)]">
                        Tier {d.tier}
                      </span>
                    </div>
                    ...
                    <div className="text-[11px] text-[#505058] space-y-0.5 pt-2 border-t border-[rgba(255,255,255,0.05)]">
                      <p><span className="text-[#44444e]">Unlock:</span> <span className="text-[#909098]">{d.unlock}</span></p>
                      <p><span className="text-[#44444e]">Loot:</span> <span className="text-[#909098]">{d.loot}</span></p>
                      <p><span className="text-[#44444e]">Boss:</span> <span className="text-amber-400/80">{d.boss}</span></p>
                    </div>
                  </div>
                </div>
              ))}
            ...
            <div>
              ...
              {[
                ...
                <div key={o.outcome} className={`border rounded-xl p-4 mb-3 ${o.bg}`}>
                  ...
                  <p className="text-[#78788a] text-xs mt-2">{o.notes}</p>
                </div>
              ))}
          ...
          {section === 'xp' && (
            <div>
              ...
              <div className="bg-[#111118] rounded-xl p-4 text-sm text-[#9090b8] space-y-1">
