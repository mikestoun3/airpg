...
function H3({ children }: { children: React.ReactNode }) {
  return <h3 className="text-red-300 font-semibold text-sm mb-2 mt-4">{children}</h3>;
}
function P({ children }: { children: React.ReactNode }) {
  return <p className="text-[#9090b8] text-sm leading-relaxed mb-2">{children}</p>;
}
function Code({ children }: { children: React.ReactNode }) {
  return <code className="bg-[#130909] border border-[rgba(200,70,70,0.2)] rounded px-1.5 py-0.5 text-red-300 text-xs font-mono">{children}</code>;
}
...
function Row({ label, value, sub }: { label: string; value: string; sub?: string }) {
  return (
    <div className="flex items-start justify-between py-2 border-b border-[rgba(200,70,70,0.08)] last:border-0">
      <span className="text-[#7a5050] text-sm">{label}</span>
      <div className="text-right">
        <span className="text-slate-200 text-sm font-mono">{value}</span>
        {sub && <div className="text-[#6a4040] text-[10px]">{sub}</div>}
      </div>
    </div>
  );
}
  ...
  return (
    <div className="flex flex-col md:flex-row gap-4 md:gap-6 md:h-full">
          ...
          <button key={s.id} onClick={() => setSection(s.id)}
            className={`flex-shrink-0 flex items-center gap-1.5 px-3 py-1.5 rounded-full text-xs border transition-all ${
              section === s.id
                ...
                : 'text-[#5a3535] border-[rgba(200,70,70,0.15)] bg-[#180c0c]'
            }`}>
            <span>{s.icon}</span>
      ...
      <div className="hidden md:block w-48 flex-shrink-0">
        <div className="bg-[#180c0c] border border-[rgba(200,70,70,0.2)] rounded-xl p-2 space-y-0.5 sticky top-0">
          {SECTIONS.map(s => (
            <button key={s.id} onClick={() => setSection(s.id)}
              className={`w-full flex items-center gap-2.5 px-3 py-2 rounded-lg text-sm transition-all text-left ${
                section === s.id
                  ? 'bg-[#1e0e0e] text-slate-100 border border-[rgba(200,70,70,0.25)]'
                  : 'text-[#5a3535] hover:bg-[#1e1010] hover:text-[#c09090]'
              }`}>
      ...
      <div className="flex-1 overflow-y-auto pr-0 md:pr-1">
        <div className="bg-[#180c0c] border border-[rgba(200,70,70,0.2)] rounded-xl p-4 md:p-6 max-w-3xl">
            ...
            <div>
              ...
              <Box color="purple">
                <ol className="space-y-2 text-sm text-[#9090b8]">
                  <li><span className="text-red-300 font-bold">1. Pick a dungeon</span> in Adventure and choose how many floors (1–10).</li>
                  <li><span className="text-red-300 font-bold">2. Send your hero</span> — each floor takes ~2 minutes. Speed stat reduces this.</li>
                  <li><span className="text-red-300 font-bold">3. Hero returns</span> — collect loot, gold, XP and materials.</li>
                  <li><span className="text-red-300 font-bold">4. Upgrade</span> — equip better gear, level up stats, build camp upgrades.</li>
                  <li><span className="text-red-300 font-bold">5. Go deeper</span> — floors scale in difficulty. Every 10th floor is a boss. Progress is saved at floor 10 checkpoints.</li>
                </ol>
              </Box>

              <H3>Key Numbers at a Glance</H3>
              <div className="bg-[#130909] rounded-xl p-4 space-y-0">
          ...
          {section === 'stats' && (
            <div>
              ...
              <Box color="purple">
                ...
                <div className="bg-[#130909] rounded-lg p-3 mt-2">
              ...
              <Box color="blue">
                ...
                <p className="text-[#5a3535] text-xs text-center mt-1">Example: PWR 8, END 5 → CR = floor(12 + 5) = 17</p>
              </Box>
            </div>
          )}
          ...
          {section === 'combat' && (
            <div>
              ...
              <div className="bg-[#130909] rounded-xl p-4">
              ...
              <Box color="blue">
                ...
                <div className="mt-3 space-y-1">
                  <p className="text-[#5a3535] text-xs text-center">CR = DC → 50% odds · CR = DC+30 → 80% odds · CR = DC+45 → 95% (cap)</p>
                </div>
              </Box>

              <H3>Examples</H3>
              <div className="bg-[#130909] rounded-xl p-4 space-y-3 text-sm text-[#9090b8]">
                <div>
          ...
          {section === 'floors' && (
            <div>
              ...
              <Box color="purple">
                ...
                <p className="text-[#5a3535] text-xs text-center mt-1">Example: Goblin Warrens floor 5 = round(5 + 4 × 2.5) = 15</p>
              </Box>
              ...
              <Box color="amber">
                ...
                <p className="text-[#5a3535] text-xs text-center mt-1">Floor 10 = +72% gold · Floor 20 = +152% gold</p>
              </Box>
            ...
            <div>
              ...
              <div className="space-y-2">
                {[
                  ...
                  <div key={r.tier} className="bg-[#130909] rounded-xl overflow-hidden">
                    <div className="px-3 py-2 border-b border-[rgba(200,70,70,0.08)] flex items-center gap-3">
                      <span className={`font-bold text-sm ${r.color}`}>T{r.tier} {r.name}</span>
                      <span className="text-[10px] text-[#6a4040]">GS {r.gs} · {r.primary} primary · {r.secondary} secondary</span>
                    </div>
                    <div className="px-3 py-2 text-xs text-[#7a5050] flex flex-col sm:flex-row gap-2">
                      <span>🔥 <span className="text-[#a07070]">{r.forge}</span></span>
                      <span>⚒ Absorbs: <span className="text-[#a07070]">{r.req}</span></span>
                    </div>
                  </div>
                ))}
              </div>
              ...
              <Box color="amber">
                ...
                <div className="bg-[#130909] rounded-lg p-3 mt-2 space-y-0">
          ...
          {section === 'dungeons' && (
            <div>
              ...
              {[
                ...
                <div key={d.name} className="mb-4 bg-[#130909] rounded-xl overflow-hidden border border-[rgba(200,70,70,0.12)]">
                  <div className={`h-1 bg-gradient-to-r ${d.color}`} />
                  <div className="p-4">
                    <div className="flex items-center gap-2 mb-3">
                      <h3 className="text-slate-100 font-bold text-base">{d.name}</h3>
                      <span className="text-[10px] text-[#6a4040] bg-[#180c0c] px-1.5 py-0.5 rounded border border-[rgba(200,70,70,0.15)]">
                        Tier {d.tier}
                      </span>
                    </div>
                    ...
                    <div className="text-[11px] text-[#5a3535] space-y-0.5 pt-2 border-t border-[rgba(200,70,70,0.08)]">
                      <p><span className="text-[#4a3030]">Unlock:</span> <span className="text-[#b07070]">{d.unlock}</span></p>
                      <p><span className="text-[#4a3030]">Loot:</span> <span className="text-[#b07070]">{d.loot}</span></p>
                      <p><span className="text-[#4a3030]">Boss:</span> <span className="text-amber-400/80">{d.boss}</span></p>
                    </div>
                  </div>
                </div>
              ))}
            ...
            <div>
              ...
              {[
                ...
                <div key={o.outcome} className={`border rounded-xl p-4 mb-3 ${o.bg}`}>
                  ...
                  <p className="text-[#7a5050] text-xs mt-2">{o.notes}</p>
                </div>
              ))}
          ...
          {section === 'xp' && (
            <div>
              ...
              <div className="bg-[#130909] rounded-xl p-4 text-sm text-[#9090b8] space-y-1">
