...
export function ResultModal({ result, leveled, newLevel, onClose }: Props) {
  ...
  return (
    <div className="fixed inset-0 bg-black/80 backdrop-blur-sm flex items-end sm:items-center justify-center z-50 p-0 sm:p-6">
      <div className="bg-[#130909] border border-[rgba(200,70,70,0.25)] rounded-t-2xl sm:rounded-2xl w-full sm:max-w-lg max-h-[92dvh] sm:max-h-[90vh] overflow-hidden flex flex-col shadow-2xl shadow-black/60">
        ...
        <div className="px-6 py-5 border-b border-[rgba(200,70,70,0.12)]">
          <div className="flex items-start justify-between">
            ...
            <div className="text-right text-xs text-[#6a4040] mt-1">
              <div>Roll: <span className={result.combatRoll >= result.dc ? 'text-emerald-400' : 'text-red-400'}>{result.combatRoll}</span></div>
              <div>Need: <span className="text-[#a07070]">{result.dc}</span></div>
            </div>
          </div>
        </div>

        {/* Rewards strip */}
        <div className="px-6 py-4 flex gap-3 border-b border-[rgba(200,70,70,0.10)]">
          <div className="flex-1 bg-[#180c0c] rounded-xl p-3 text-center">
            <p className="text-amber-400 font-bold text-xl">+{result.goldGained}</p>
            <p className="text-[#5a3535] text-[11px] mt-0.5">Gold</p>
          </div>
          {result.essenceGained > 0 && (
            <div className="flex-1 bg-[#180c0c] rounded-xl p-3 text-center">
              <p className="text-rose-400 font-bold text-xl">+{result.essenceGained}</p>
              <p className="text-[#5a3535] text-[11px] mt-0.5">Essence</p>
            </div>
          )}
          <div className="flex-1 bg-[#180c0c] rounded-xl p-3 text-center">
            <p className="text-blue-400 font-bold text-xl">+{result.xpGained}</p>
            <p className="text-[#5a3535] text-[11px] mt-0.5">XP</p>
          </div>
        </div>
        ...
        <div className="flex-1 overflow-y-auto px-6 py-4 space-y-4">
                      ...
                      {floor.items.length > 0 && (
                        <div className="flex flex-wrap gap-1 mt-1.5">
                          {floor.items.map((item) => (
                            <span
                              key={item.id}
                              className="text-[11px] px-1.5 py-0.5 rounded bg-[#130909]/60"
          ...
          {result.loot.length > 0 ? (
            ...
            !hasFloorResults && (
              <div className="text-center py-6 text-[#6a4040]">
              ...
              <div className="flex flex-wrap gap-2">
                {result.resourcesGained.map((r) => (
                  <div key={r.resourceId}
                    className="flex items-center gap-1.5 bg-[#130909] border border-[rgba(200,70,70,0.15)] rounded-lg px-3 py-1.5 text-sm">
        ...
        <div className="px-6 py-4 border-t border-[rgba(200,70,70,0.10)]">
