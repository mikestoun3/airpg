...
function NicknameModal({ onDone }: { onDone: () => void }) {
  ...
  return (
    <div className="fixed inset-0 z-50 bg-[#0d0808]/95 backdrop-blur-sm flex items-center justify-center px-4">
      <div className="w-full max-w-sm bg-[#160c0c] border border-[rgba(200,70,70,0.25)] rounded-2xl overflow-hidden shadow-2xl">
        <div className="px-6 pt-8 pb-6">
          <div className="text-center mb-6">
            ...
            <p className="text-[#6a4040] text-sm mt-1">Это имя увидят другие игроки</p>
          </div>
          ...
          <div className="relative mb-1.5">
            <input
              ...
              maxLength={13}
              className={`w-full bg-[#1c0e0e] border rounded-xl px-4 py-3 text-slate-100 text-sm font-mono tracking-wide outline-none transition-colors ${
                error ? 'border-red-500/60 focus:border-red-500' : 'border-[rgba(200,70,70,0.25)] focus:border-red-600'
              }`}
            />
            <span className={`absolute right-3 top-1/2 -translate-y-1/2 text-xs font-mono ${value.length >= 13 ? 'text-amber-400' : 'text-[#4a3030]'}`}>
              {value.length}/13
            </span>
          </div>
          ...
          {error ? (
            ...
            <p className="text-[#4a3030] text-xs mb-4 px-1">Латиница, цифры и _ · 3–13 символов · навсегда</p>
          )}
...
export default function GamePage() {
    ...
    return (
      <div className="h-[100dvh] flex items-center justify-center bg-[#0d0808]">
        <div className="flex flex-col items-center gap-4">
          ...
          <p className="text-[#6a4040] text-sm">Entering the dungeon...</p>
        </div>
      </div>
    );
  }
    ...
    return (
      <div className="h-[100dvh] flex items-center justify-center bg-[#0d0808]">
        <div className="flex flex-col items-center gap-4 text-center px-6">
          ...
          <p className="text-[#6a4040] text-sm max-w-xs">Сервер временно недоступен. Мы уже всё чиним — возвращайтесь позже.</p>
        </div>
      </div>
    );
  }
    ...
    return (
      <div className="h-[100dvh] flex items-center justify-center bg-[#0d0808]">
        <div className="flex flex-col items-center gap-5 text-center px-6 max-w-sm">
          ...
          {banned.reason && (
            <p className="text-[#7a5050] text-sm bg-[#160c0c] border border-[rgba(200,70,70,0.2)] rounded-xl px-4 py-3">{banned.reason}</p>
          )}
          <p className="text-[#6a4040] text-sm">Если вы считаете, что блокировка была ошибочной — свяжитесь с нами.</p>
  ...
  return (
    <div className="h-[100dvh] flex overflow-hidden bg-[#0d0808]">
      ...
      <aside className="hidden md:flex w-56 flex-shrink-0 bg-[#110a0a] border-r border-[rgba(200,70,70,0.12)] flex-col">
        <div className="px-5 py-5 border-b border-[rgba(200,70,70,0.10)]">
          <div className="flex items-center gap-2">
            ...
            <div>
              ...
              <p className="text-[#6a4040] text-[10px] tracking-widest uppercase">Idle Dungeon</p>
            </div>
          </div>
        </div>
        ...
        <nav className="flex-1 overflow-y-auto py-4 px-2">
          {NAV.map((item, idx) => {
            ...
            return (
              <div key={item.id}>
                {showSection && (
                  <p className="text-[10px] text-[#4a3030] uppercase tracking-widest px-3 mt-3 mb-1.5 flex items-center gap-1.5">
                ...
                <button onClick={() => setActiveTab(item.id)}
                  className={`w-full flex items-center gap-3 px-3 py-2.5 rounded-xl text-sm font-medium transition-all relative mb-0.5 ${
                    isActive
                      ? 'bg-gradient-to-r from-[#220e0e] to-[#1a0d0d] text-slate-100 border border-[rgba(200,70,70,0.25)]'
                      : 'text-[#7a5050] hover:bg-[#180d0d] hover:text-[#b07070]'
                  }`}>
                  {isActive && <span className="absolute left-0 top-1/2 -translate-y-1/2 w-0.5 h-5 bg-red-600 rounded-r-full" />}
        ...
        <div className="px-3 py-3 border-t border-[rgba(200,70,70,0.10)] space-y-2">
          <div className="flex items-center gap-2 px-2 py-2 rounded-xl bg-[#180c0c]">
            ...
            <div className="flex-1 min-w-0">
              ...
              <p className="text-[#6a4040] text-[10px]">Lv.{character.level} Wanderer</p>
            </div>
          </div>
          <div className="flex items-center justify-between px-2 py-1.5 rounded-lg bg-[#110a0a] border border-[rgba(200,70,70,0.08)]">
            <span className="text-[10px] text-[#4a3030] font-mono">{shortAddr(state.walletAddress)}</span>
            <button onClick={handleLogout} className="text-[10px] text-[#4a2a2a] hover:text-[#7a5050] transition-colors">out</button>
          </div>
        </div>
      </aside>
      ...
      <div className="flex-1 flex flex-col overflow-hidden min-w-0">
        ...
        {/* Top bar */}
        <header className="h-12 md:h-14 flex-shrink-0 bg-[#110a0a] border-b border-[rgba(200,70,70,0.12)] flex items-center justify-between px-3 md:px-6 relative">
          <div className="flex items-center gap-2">
            {/* Mobile: mini logo */}
          ...
          <div className="flex items-center gap-1">
            <div className="flex items-center gap-1.5 px-2 md:px-3 py-1 md:py-1.5 bg-[#180c0c] rounded-xl border border-[rgba(200,70,70,0.12)]">
            ...
            {character.essence > 0 && (
              <div className="hidden sm:flex items-center gap-1.5 px-2 md:px-3 py-1 md:py-1.5 bg-[#180c0c] rounded-xl border border-[rgba(200,70,70,0.12)]">
                <span className="text-xs">🔮</span>
                <span className="text-rose-400 font-bold text-xs md:text-sm">{character.essence}</span>
              </div>
            )}
            <div className="flex items-center gap-1 md:gap-2 px-2 md:px-3 py-1 md:py-1.5 bg-[#180c0c] rounded-xl border border-[rgba(200,70,70,0.12)]">
              <span className="text-[#5a3535] text-[10px] md:text-xs">Lv.</span>
              <span className="text-slate-200 font-bold text-xs md:text-sm">{character.level}</span>
              <div className="hidden sm:block w-14 md:w-20 h-1.5 bg-[#200e0e] rounded-full overflow-hidden">
                <div className="h-full bg-gradient-to-r from-amber-500 to-yellow-400 rounded-full transition-all" style={{ width: `${xpPct}%` }} />
              </div>
            </div>
            {/* Mobile logout */}
            <button onClick={handleLogout} className="md:hidden ml-1 text-[10px] text-[#4a2a2a] hover:text-[#7a5050] px-1.5 py-1 rounded">out</button>
          </div>
        </header>
                  ...
                  {group.tabs.map((tabId, i) => (
                    <button key={tabId} onClick={() => setActiveTab(tabId)}
                      className={`px-3.5 py-1.5 rounded-lg text-xs font-semibold transition-all border ${
                        activeTab === tabId
                          ? 'bg-[#1e0e0e] text-slate-100 border-[rgba(200,70,70,0.35)]'
                          : 'text-[#6a4040] border-transparent hover:text-[#b07070]'
                      }`}>
                      {group.subs[i]}
                    </button>
                  ))}
        ...
        <nav className="md:hidden flex-shrink-0 bg-[#110a0a] border-t border-[rgba(200,70,70,0.12)] flex items-stretch">
            ...
            return (
              <button key={group.id}
                ...
                className={`flex-1 flex flex-col items-center justify-center py-2 gap-0.5 relative transition-colors ${
                  isActive ? 'text-slate-100' : 'text-[#4a3030]'
                }`}>
                {isActive && <span className="absolute top-0 left-1/2 -translate-x-1/2 w-6 h-0.5 bg-red-600 rounded-full" />}
